...
GOOD = """[WRAPPER_DIAG] --- ExynosTools LSFG integration ---
...
[WRAPPER_DIAG] ================ WRAPPER DIAGNOSTICS ================
...
class RuntimeDiagContractTest(unittest.TestCase):
    ...
    def test_rejects_bad_latest_run_even_if_older_appended_run_passed(self) -> None:
        bad_latest = GOOD.replace("backend: system-vulkan", "backend: adrenotools-custom")
        result = self.run_diag(GOOD + bad_latest)
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("system Vulkan backend", result.stdout + result.stderr)
