...
int main(void)
{
    ...
    for (unsigned i = 0;
        ...
        /*
         * Match the shader's texture sampling:
         *
         *   uvA = uv - motion * 0.5
         *   uvB = uv + motion * 0.5
         *
         * With texel-center UVs, the horizontal displacement is:
         *
         *   shift = motion.x * W * 0.5
         *
         * Vulkan linear filtering interpolates between adjacent texels
         * and clamp-to-edge handles the boundaries.
         */
        const float motionX = -0.10f;
        const float shift = motionX * (float)W * 0.5f;

        float sampleA = (float)x - shift;
        float sampleB = (float)x + shift;

        if (sampleA < 0.0f)
            sampleA = 0.0f;
        if (sampleA > (float)(W - 1))
            sampleA = (float)(W - 1);

        if (sampleB < 0.0f)
            sampleB = 0.0f;
        if (sampleB > (float)(W - 1))
            sampleB = (float)(W - 1);

        unsigned a0 = (unsigned)sampleA;
        unsigned a1 = a0 + 1 < W ? a0 + 1 : a0;
        float af = sampleA - (float)a0;

        unsigned b0 = (unsigned)sampleB;
        unsigned b1 = b0 + 1 < W ? b0 + 1 : b0;
        float bf = sampleB - (float)b0;

        uint8_t aR0 =
            (uint8_t)((a0 * 255u + 31u) / 63u);

        uint8_t aR1 =
            (uint8_t)((a1 * 255u + 31u) / 63u);

        uint8_t bR0 =
            255u - (uint8_t)((b0 * 255u + 31u) / 63u);

        uint8_t bR1 =
            255u - (uint8_t)((b1 * 255u + 31u) / 63u);
        ...
        /*
         * Motion is horizontal only. Therefore both samples use the
         * same Y coordinate. Frame A and B have complementary G values,
         * so their midpoint is approximately 0x80.
         */
        uint8_t expectedG = 0x80;
        uint8_t expectedB = 0x80;
        uint8_t expectedA = 0xff;
    ...
    printf("\n=== TWO-FRAME INTERPOLATION RESULT ===\n");

    if (failures == 0) {
        printf("RESULT: PASS\n");
        printf("TWO INPUT IMAGES -> COMPUTE -> OUTPUT IMAGE: YES\n");
        printf("GPU imageLoad(A) + imageLoad(B) + arithmetic + imageStore: YES\n");
    } else {
        printf("RESULT: FAIL\n");
        printf("Failures: %d\n", failures);
    }
