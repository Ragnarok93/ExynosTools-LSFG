...
int main(void)
{
     ...
     */
    /*
     * Phase 2B two-frame sampler oracle.
     *
     * Frame A:
     *   R = x-gradient
     *   G = y-gradient
     ...
     *
     * Frame B:
     *   R = inverse x-gradient
     *   G = inverse y-gradient
     *   B = 0xe0
     *   A = 0xff
     *
     * The shader samples both frames and averages them:
     *
     *   result = A * 0.5 + B * 0.5
     *
     * At pixel (0,0), the motion-adjusted samples remain close
     * to the clamped edge, so R/G should be approximately 0x80.
     *
     * B should be the midpoint of 0x20 and 0xe0:
     *
     *   (0x20 + 0xe0) / 2 = 0x80
     *
     * Alpha should also remain 0xff because both source alpha
     * channels are 0xff.
     */

    size_t o = 0;

    const uint8_t expectedR = 0x80;
    const uint8_t expectedG = 0x80;
    const uint8_t expectedB = 0x80;
    const uint8_t expectedA = 0xff;

    int failures = 0;

    /*
     * Allow one LSB of tolerance for normalized conversion.
     */
    int ok =
        mapped[o + 0] >= expectedR - 1 &&
        mapped[o + 0] <= expectedR + 1 &&
        mapped[o + 1] >= expectedG - 1 &&
        mapped[o + 1] <= expectedG + 1 &&
        mapped[o + 2] >= expectedB - 1 &&
        mapped[o + 2] <= expectedB + 1 &&
        mapped[o + 3] >= expectedA - 1 &&
        mapped[o + 3] <= expectedA + 1;
