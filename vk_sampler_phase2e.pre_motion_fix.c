...
int main(void)
{
    ...
    /*
     * Phase 2D spatial validation.
     *
     * The shader now calculates a different UV coordinate for every
     * invocation:
     *
     *   A = sample(frameA, uv - 0.5px)
     *   B = sample(frameB, uv + 0.5px)
     *   result = mix(A, B, 0.5)
     *
     * Check representative pixels throughout the image rather than
     * validating only [0,0].
     */
    ...
    /*
     * RGBA8 normalized-format sampling/interpolation may differ by
     * several LSBs depending on the implementation's filtering and
     * normalized conversion precision.
     *
     * Observed Xclipse 940 result:
     *   R = 0x7d..0x7f
     *   expected = 0x80
     *
     * Accept a conservative +/-4 LSB tolerance while still rejecting
     * meaningful spatial/interpolation errors.
     */
    ...
    for (unsigned i = 0;
        ...
        /*
         * A spatially varying shader should preserve the synthetic
         * gradient midpoint throughout the image.
         *
         * Permit a small normalized-format rounding tolerance.
         */
        int ok =
    ...
    if (failures == 0) {
        printf("RESULT: PASS\n");
        printf("SPATIAL UV GENERATION: YES\n");
        printf("MULTI-PIXEL FRAME A/B SAMPLING: YES\n");
        printf("PER-PIXEL INTERPOLATION: YES\n");
    } else {
        printf("RESULT: FAIL\n");
        printf("Failures: %u\n", failures);
    }
